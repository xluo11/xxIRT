...
The effect of sample sizes and test lengths on estimation
...
+ `mst_obj(x, theta, indices, target, flatten)`: Add objective functions to the assembly job. Use `theta` to specify at which $\theta$ points the information is optimized. When `target` is `NULL`, the information is maximized at $\theta$ points; otherwise, the information approaches the given targets. Use `flatten` argument to obtain a flatter TIF by curbing the information difference between $\theta$ points. `indices` sets on which modules or routes the objective functions are added.
...
+ `mst_module_mininfo(x, theta, mininfo, indices)`:  Set the minimum information at given $\theta$ points for some modules.
