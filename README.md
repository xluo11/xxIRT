...
-   `ata_solve(x, as.list, timeout, ...)`: Solve the ATA job. Use `as.list=TRUE` to return results as list; otherwise, data frame. Use `timeout` to set the time limits of the job in seconds. Pass additional control parameters in `...`. See the documentation of *lp\_solve* and *lpSolveAPI* for more details. Once solved, the ATA job is added with four objects: `status` (status code of the solution), `optimum` (final optimal value of the objective function), `result` (a binary matrix of assembly results), and `item` (a list of data frame of assembled test forms).
